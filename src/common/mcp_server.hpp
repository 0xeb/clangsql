...
/**
 * ClangsqlMCPServer - MCP server for CLANGSQL
 *
 * Thread-safe MCP server using command queue pattern.
 * Tool handlers queue commands for execution on the main thread.
 *
 * Usage modes:
 * 1. CLI: Call run_until_stopped() to process commands on main thread
 * 2. Direct: Use callbacks directly (no run_until_stopped() needed)
 */
...
class ClangsqlMCPServer {
public:
    ClangsqlMCPServer();
    ~ClangsqlMCPServer();

    // Non-copyable
    ClangsqlMCPServer(const ClangsqlMCPServer&) = delete;
    ClangsqlMCPServer& operator=(const ClangsqlMCPServer&) = delete;
