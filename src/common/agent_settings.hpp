...
// Agent settings stored in ~/.clangsql/agent_settings.json
...
// Get the settings directory path (~/.clangsql or %APPDATA%\clangsql)
inline std::string GetSettingsDir() {
#ifdef _WIN32
    char path[MAX_PATH];
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_APPDATA, nullptr, 0, path))) {
        return std::string(path) + "\\clangsql";
    }
    // Fallback to USERPROFILE
    const char* userprofile = std::getenv("USERPROFILE");
    if (userprofile) {
        return std::string(userprofile) + "\\.clangsql";
    }
    return ".clangsql";
#else
    ...
    if (home) {
        return std::string(home) + "/.clangsql";
    }
    return ".clangsql";
#endif
}
