...
static const char* HTTP_HELP_TEXT = R"(CLANGSQL HTTP REST API
=====================

SQL interface for C/C++ source code via HTTP.
...
class ClangsqlHTTPServer::Impl {
public:
    httplib::Server svr;
    std::thread server_thread;
};

ClangsqlHTTPServer::ClangsqlHTTPServer() = default;

ClangsqlHTTPServer::~ClangsqlHTTPServer() {
    stop();
}

std::string ClangsqlHTTPServer::queue_and_wait(const std::string& sql) {
    if (!running_.load()) {
...
int ClangsqlHTTPServer::start(int port, HTTPQueryCallback query_cb,
    ...
    svr.Get("/", [port](const httplib::Request&, httplib::Response& res) {
        std::string welcome = "CLANGSQL HTTP Server (REPL)\n\nEndpoints:\n"
            "  GET  /help     - API documentation\n"
            "  POST /query    - Execute SQL query\n"
            "  GET  /status   - Health check\n"
            "  POST /shutdown - Stop server\n\n"
            "Example: curl -X POST http://localhost:" + std::to_string(port) +
            "/query -d \"SELECT name FROM functions LIMIT 5\"\n";
...
void ClangsqlHTTPServer::set_interrupt_check(std::function<bool()> check) {
    interrupt_check_ = check;
}

void ClangsqlHTTPServer::run_until_stopped() {
    while (running_.load()) {
...
void ClangsqlHTTPServer::complete_pending_commands(const std::string& result) {
...
std::string ClangsqlHTTPServer::url() const {
...
std::string format_http_info(int port) {
    ...
    ss << "Example:\n";
    ss << "  curl -X POST http://127.0.0.1:" << port << "/query -d \"SELECT name FROM functions LIMIT 5\"\n\n";
